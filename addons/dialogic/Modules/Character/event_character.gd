...
enum CharacterActions {JOIN, LEAVE, UPDATE}

### Settings

## The type of action of this event (JOIN/LEAVE/UPDATE). See [CharacterActions].
var action :=  CharacterActions.JOIN
...
func _execute() -> void:
	...
	# JOIN -------------------------------------
	if action == CharacterActions.JOIN:
	...
	# LEAVE -------------------------------------
	elif action == CharacterActions.LEAVE:
		if character_identifier == '--All--':
	...
	# UPDATE -------------------------------------
	elif action == CharacterActions.UPDATE:
...
func to_text() -> String:
	...
	# ACTIONS
	match action:
		CharacterActions.JOIN: result_string += "join "
		CharacterActions.LEAVE: result_string += "leave "
		CharacterActions.UPDATE: result_string += "update "

	var default_values := DialogicUtil.get_custom_event_defaults(event_name)

	# CHARACTER IDENTIFIER
	if action == CharacterActions.LEAVE and character_identifier == '--All--':
		result_string += "--All--"
	elif character:
		...
		if portrait.strip_edges() != default_values.get('portrait', ''):
			if action != CharacterActions.LEAVE and (action != CharacterActions.UPDATE or set_portrait):
				result_string += " (" + portrait + ")"

	# TRANSFORM
	if action == CharacterActions.JOIN or (action == CharacterActions.UPDATE and set_transform):
		result_string += " " + str(transform)

	# SETS:
	if action == CharacterActions.JOIN or action == CharacterActions.LEAVE:
...
func from_text(string:String) -> void:
	...
	match result.get_string('type'):
		"join": action = CharacterActions.JOIN
		"leave": action = CharacterActions.LEAVE
		"update": action = CharacterActions.UPDATE
	...
	if given_name:
		if action == CharacterActions.LEAVE and given_name == "--All--":
	...
	return {
		...
		"action" 		: {"property": "action", 					"default": 0, "custom_stored":true,
							"suggestions": func(): return {'Join':
										{'value':CharacterActions.JOIN},
										'Leave':{'value':CharacterActions.LEAVE},
										'Update':{'value':CharacterActions.UPDATE}}},
...
func build_event_editor() -> void:
	...
	add_header_edit('set_portrait', ValueType.BOOL_BUTTON,
			{'icon':load("res://addons/dialogic/Modules/Character/update_portrait.svg"),
			 'tooltip':'Change Portrait'}, "should_show_portrait_selector() and action == CharacterActions.UPDATE")
	add_header_edit('portrait', ValueType.DYNAMIC_OPTIONS,
			...
			'should_show_portrait_selector() and (action != CharacterActions.UPDATE or set_portrait)')
	add_header_edit('set_transform', ValueType.BOOL_BUTTON,
			{'icon': load("res://addons/dialogic/Modules/Character/update_position.svg"), 'tooltip':'Change Position'}, "character != null and !has_no_portraits() and action == CharacterActions.UPDATE")
	add_header_label('at position', 'character != null and !has_no_portraits() and action == CharacterActions.JOIN')
	add_header_label('to position', 'character != null and !has_no_portraits() and action == CharacterActions.UPDATE and set_transform')
	add_header_edit('transform', ValueType.DYNAMIC_OPTIONS,
			...
			'character != null and !has_no_portraits() and action != %s and (action != CharacterActions.UPDATE or set_transform)' %CharacterActions.LEAVE)
	...
	add_body_edit('animation_repeats', ValueType.NUMBER, {'left_text':'Repeat:', 'mode':1, "min":1},
			'should_show_animation_options() and !animation_name.is_empty() and action == %s)' %CharacterActions.UPDATE)
	add_body_line_break()
	...
	add_body_edit('set_z_index', ValueType.BOOL_BUTTON, {'icon':load("res://addons/dialogic/Modules/Character/update_z_index.svg"), 'tooltip':'Change Z-Index'}, "character != null and action == CharacterActions.UPDATE")
	add_body_edit('z_index', ValueType.NUMBER, {'left_text':'Z-index:', 'mode':1},
			'action != %s and (action != CharacterActions.UPDATE or set_z_index)' %CharacterActions.LEAVE)
	add_body_edit('set_mirrored', ValueType.BOOL_BUTTON, {'icon':load("res://addons/dialogic/Modules/Character/update_mirror.svg"), 'tooltip':'Change Mirroring'}, "character != null and action == CharacterActions.UPDATE")
	add_body_edit('mirrored', ValueType.BOOL, {'left_text':'Mirrored:'},
			'action != %s and (action != CharacterActions.UPDATE or set_mirrored)' %CharacterActions.LEAVE)
	add_body_edit('extra_data', ValueType.SINGLELINE_TEXT, {'left_text':'Extra Data:'}, 'action != CharacterActions.LEAVE')


func should_show_transform_options() -> bool:
	return action == CharacterActions.UPDATE and set_transform
...
func should_show_fade_options() -> bool:
	return action == CharacterActions.UPDATE and set_portrait and character and not character.portraits.is_empty()


func should_show_portrait_selector() -> bool:
	return character and len(character.portraits) > 1 and action != CharacterActions.LEAVE
...
func get_character_suggestions(search_text:String) -> Dictionary:
	return DialogicUtil.get_character_suggestions(search_text, character, false, action == CharacterActions.LEAVE, editor_node)
